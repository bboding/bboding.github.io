...
```
예시
select date_format(createdAt, '%y%m%d') as date, count(*) from Logs where logId = 33 group by date order by date asc;
```

## react-native svg dynamic stroke color 동적으로 넣기
...
위에서 MyCon 컴포넌트에서 color를 넣어준 값이 아래의 currentColor 로 들어간다.
